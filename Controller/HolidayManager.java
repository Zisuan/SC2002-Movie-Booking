...
public class HolidayManager {
    ...
    // public void updateHoliday(int updateCase, ArrayList<Holiday> holidayDB, Date oldHolidayDate, Date newHolidayDate, String newHolidayName){
    ...
    public void listHolidays(ArrayList<Holiday> holidayDB) throws ParseException{
        ...
        for (int i = 0; i < holidayDB.size(); i++) {
            System.out.printf("%s   %s\n", holidayDB.get(i).getHolidayName(), dh.dateToString(holidayDB.get(i).getHolidayDate()));
        }
    }
    
    public void saveHolidays(String fileName, ArrayList<Holiday>holidayDB)throws IOException {
        DatabaseManager.writeSerializedObject(fileName, holidayDB);
    }

    public ArrayList<Holiday> loadHolidays(String fileName)throws IOException, SecurityException, ClassNotFoundException{
